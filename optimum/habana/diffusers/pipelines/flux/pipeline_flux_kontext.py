...
class GaudiFluxKontextPipeline(GaudiDiffusionPipeline, FluxKontextPipeline):
    ...
    @classmethod
    def _split_inputs_into_batches(cls, batch_size, latents, image_latents, prompt_embeds, pooled_prompt_embeds, negative_prompt_embeds, negative_pooled_prompt_embeds, guidance):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        (
            ...
            guidance_batches,
            num_dummy_samples,
        ) = self._split_inputs_into_batches(batch_size, latents, image_latents, prompt_embeds, pooled_prompt_embeds, negative_prompt_embeds, negative_pooled_prompt_embeds, guidance)
        ...
        for j in range(num_batches):
            ...
            if do_true_cfg:
                negative_prompt_embeds_batch = negative_prompt_embeds_batches[0].squeeze()
                negative_prompt_embeds_batches = torch.roll(negative_prompt_embeds_batches, shifts=-1, dims=0)
                negative_pooled_prompt_embeds_batch = negative_pooled_prompt_embeds_batches[0].squeeze()
            ...
            for i in self.progress_bar(range(len(timesteps))):
                ...
                if image_latents_batch is not None:
                    latent_model_input = torch.cat([latents_batch, image_latents_batch], dim=1)
